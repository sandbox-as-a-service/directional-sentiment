...
export function makePollFeedSource(allItems: Array<PollFeedPageItem>): PollFeedSource {
...
describe("getPollFeed", () => {
  describe("without cursor", () => {
    it("returns 20 items and a nextCursor when more exist", async () => {
      ...
      const result = await getPollFeed({pollFeed, input: {}})
  ...
  describe("with cursor", () => {
    it("returns the next page after the cursor", async () => {
      ...
      const result = await getPollFeed({pollFeed, input: {limit: 3, cursor}})
  ...
  describe("limit handling", () => {
    it("honors a smaller requested limit", async () => {
      ...
      const result = await getPollFeed({pollFeed, input: {limit: 5}})
    ...
    it("clamps an excessive limit to 50", async () => {
      ...
      const result = await getPollFeed({pollFeed, input: {limit: 100}})
    ...
    it("floors invalid limits to 1", async () => {
      ...
      const result = await getPollFeed({pollFeed, input: {limit: 0}})
